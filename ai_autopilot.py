...
PRICE_GUIDE = os.environ.get("KWORK_PRICE_GUIDE", (
    "Базовые цены исполнителя (это НИЖНИЕ опорные точки — от них прыгаем ВВЕРХ "
    "по мере усложнения):\n"
    "- Простой Telegram-бот — максимум ~10 000 ₽.\n"
    "- Лендинг / сайт-визитка с базовым SEO и запуском — от 25 000 ₽.\n"
    "- Сайт-каталог / интернет-магазин (каталог, корзина, онлайн-оплата, "
    "фильтры, базовое SEO) — от 80 000 ₽ и выше по объёму.\n"
    "Логика: определи тип проекта, возьми ближайшую базовую ставку и масштабируй "
    "ВВЕРХ по сложности (доп. функции, интеграции, личный кабинет, оплата, много "
    "страниц/каталог, API, нагрузки — тем дороже). Ниже базовой ставки для "
    "соответствующего типа проекта НЕ опускайся. Думай как инженер, цена обоснованная."
))
...
def ai_compose(project: dict) -> dict:
    ...
    sys_prompt = (
        ...
        "ЦЕНА: оцени РЕАЛЬНУЮ сложность задачи и назначь адекватную цену, "
        "отталкиваясь от прайс-ориентиров исполнителя (ниже). Думай как инженер: "
        "определи тип проекта, найди близкий ориентир и масштабируй по сложности "
        "(больше функций/интеграций/страниц/каталог/оплата/SEO — выше; простая "
        "доработка — ниже). НЕ копируй слепо желаемую заказчиком цену и НЕ ставь "
        "ни символических, ни космических сумм — цена должна быть обоснованной. "
        f"\n{PRICE_GUIDE}\n"
