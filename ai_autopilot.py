...
# Прайс-ориентиры исполнителя (опорные точки). ИИ оценивает сложность заявки
# и подбирает цену, отталкиваясь от этих ставок (масштабирует вверх/вниз).
# Редактируется через env KWORK_PRICE_GUIDE (своим текстом) либо тут.
PRICE_GUIDE = os.environ.get("KWORK_PRICE_GUIDE", (
    "Ориентиры цен за ПОЛНЫЙ проект «под ключ» соответствующего типа "
    "(это ТИПИЧНЫЕ суммы за готовый продукт, НЕ за мелкую задачу):\n"
    "- Простой Telegram-бот «под ключ» — около 10 000 ₽ (потолок для простого бота).\n"
    "- Лендинг / сайт-визитка «под ключ» с базовым SEO и запуском — около 25 000 ₽.\n"
    "- Сайт-каталог / интернет-магазин (каталог, корзина, онлайн-оплата, "
    "фильтры, базовое SEO) — от 80 000 ₽ и выше по объёму.\n"
    "Это референсные точки, А НЕ минимум для любой заявки. Если задача — "
    "небольшая доработка/часть проекта (правка, одна функция, интеграция) — "
    "ставь ПРОПОРЦИОНАЛЬНО НИЖЕ этих сумм. Если сложнее/объёмнее обычного "
    "(доп. функции, интеграции, личный кабинет, API, нагрузки) — выше."
))
...
def ai_compose(project: dict) -> dict:
    ...
    sys_prompt = (
        ...
        "ЦЕНА — рассуждай как опытный исполнитель. Цель — НЕ заломить и НЕ "
        "продешевить, а назначить СПРАВЕДЛИВУЮ рыночную цену, за которую реально "
        "берёшься и которая конкурентна. Алгоритм: "
        "(1) оцени РЕАЛЬНЫЙ объём и сложность: это полный проект «под ключ» или "
        "небольшая доработка/часть? Соотнеси с референсными ценами ниже и "
        "масштабируй В ЛЮБУЮ сторону: меньше объёма — дешевле, больше — дороже. "
        "(2) Учти рынок: желаемая заказчиком цена и допустимый бюджет показывают "
        "его ожидания, число конкурентов — конкуренцию; цена должна быть адекватной. "
        "(3) НЕ завышай цену только потому, что бюджет большой; НЕ занижай до "
        "символической; НЕ копируй слепо желаемую цену. Цена обоснованная. "
        f"\n{PRICE_GUIDE}\n"
        "Верни СТРОГО JSON с полями: "
        "description (текст предложения по правилам выше), "
        "price (целое число рублей — твоя оценка по сложности), "
        "price_reason (1 короткая фраза: почему такая цена, к какому ориентиру отнёс), "
    ...
    # Цену оценивает ИИ по сложности (с опорой на прайс-ориентиры). Система лишь
    # валидирует: не ниже PRICE_FLOOR; если ИИ не дал число — запасной расчёт.
    try:
        ai_price = int(round(float(data.get("price")) / 10.0) * 10)
